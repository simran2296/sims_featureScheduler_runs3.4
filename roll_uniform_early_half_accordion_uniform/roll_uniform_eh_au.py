...
class LinearAccordion(BasePixelEvolution):
    def __init__(self, strength=1, **kwargs):
        super().__init__(**kwargs)
        self.rise, self.strength = [
            np.array(a)
            for a in np.broadcast_arrays(np.atleast_1d(self.rise), np.atleast_1d(strength))
        ]

    def _linear_accordion(self, t, strength, base_rate):
        res = np.zeros_like(t)
        msk = t < 0.5
        res[msk] = 0

        peak = base_rate * (1.0 + strength)
        trough = base_rate * (1.0 - strength)

        msk = (t >= 0.5) & (t < 0.75)
        w = (t[msk] - 0.5) / (0.75 - 0.5)
        res[msk] = (0.75 - 0.5) * (
            w * w * peak[msk] / 2 + w * trough[msk] - w * w * trough[msk] / 2
        )

        msk = t >= 0.75
        w = (t[msk] - 0.75) / (1.0 - 0.75)
        res[msk] = (1.0 - 0.75) * (
            w * peak[msk] - w * w * peak[msk] / 2 + w * w * trough[msk] / 2
        ) + (0.75 - 0.5) * (peak[msk] + trough[msk]) / 2
    ...
    def __call__(self, mjd_in, phase):
        ...
        dt = t % self.period
        dt_scaled = dt / self.period

        steps = np.array(self.rise)
        plateus = np.cumsum(steps) - steps[0]
        result = plateus[season]

        strength = self.strength[season+1]
        base_rate = steps[season+1]
        result += self._linear_accordion(dt_scaled, strength, base_rate)
        return result
...
def make_rolling_footprints(
    ...
    for i in range(len(all_slopes)):
        _acc_strength = [
            ...
            for slope in all_slopes[i]
        ]
        step_func = LinearAccordion(
            rise=all_slopes[i],
            strength=_acc_strength
        )
