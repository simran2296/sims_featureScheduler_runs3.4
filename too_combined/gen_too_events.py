...
def gen_gw_events(mjd_start=61557, mjd_end=62502, scale=3, seed=42):
    ...
    for case in case_2_area:
        ...
        event_table = np.zeros(n, dtype=list(zip(names, types)))
...
def gen_all_events(scale_up=1):
    events = []
    events.append(gen_gw_events(scale=3*scale_up))
    events.append(gen_bbh_events(scale=3*scale_up))
    events.append(gen_lensed_BNS())
    events.append(gen_neutrino_events())
    events.append(gen_sso_events())

    results = np.concatenate(events)
    return results
