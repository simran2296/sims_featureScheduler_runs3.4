    ...
    def _check_list(self, conditions):
        ...
        if self.obs_wanted is not None:
            ...
            if np.size(in_time_window) > 0:
                ...
                match2 = np.isin(
                    self.obs_wanted["filter"][matches], conditions.current_filter
                )
...
def example_scheduler(args):
    ...
    nside = args.nside
    ddf_flush_length = args.ddf_flush_length
    ...
    fileroot, extra_info = set_run_info(
        dbroot=dbroot, file_end="fl%i_v3.4_" % ddf_flush_length, out_dir=out_dir
    )
...
def sched_argparser():
    ...
    parser.add_argument("--ddf_flush_length", type=float, default=2.0)
