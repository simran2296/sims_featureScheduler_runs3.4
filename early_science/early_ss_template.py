...
def generate_ss_blobs(nside,
        ...
        repeat_weight=-20,
        footprint_mask=None):
    """ Some blobs to get just the NES early in year 1
    """
    ...
    for filtername, filtername2 in zip(filter1s, filter2s):
        ...
        basis_functions = [val[0] for val in bfs]
        if filtername2 is None:
            survey_name = "early_ss_%i, %s" % (pair_time, filtername)
        else:
            survey_name = "early_ss_%i, %s%s" % (pair_time, filtername, filtername2)
        if filtername2 is not None:
            detailer_list.append(detailers.TakeAsPairsDetailer(filtername=filtername2))
...
def example_scheduler(args):
    ...
    ss_blobs = generate_ss_blobs(nside,
                                 footprints=ss_footprints,
                                 mjd_start=mjd_start,
                                 filter1s=['g', 'r', 'i', 'z'],
