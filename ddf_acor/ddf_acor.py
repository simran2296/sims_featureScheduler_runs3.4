...
def example_scheduler(args):
    ...
    nside = args.nside
    low_season_fraction = args.low_season_fraction
    low_season_rate = args.low_season_rate
    ...
    fileroot, extra_info = set_run_info(
        dbroot=dbroot, file_end="sf%i_lsf%i_lsr%i_v3.4_" % (ddf_season_frac*100, low_season_fraction*100, low_season_rate*100),
        out_dir=out_dir
    )
...
def sched_argparser():
    ...
    parser.add_argument("--low_season_fraction", type=float, default=0.3)
    parser.add_argument("--low_season_rate", type=float, default=0.3)
